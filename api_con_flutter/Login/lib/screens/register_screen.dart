...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  String convertirFecha(String fecha) {
    final DateTime fecha1 = DateFormat('dd/MM/yy').parse(fecha);
    return DateFormat('yyyy-MM-dd').format(fecha1);
  }
  ...
  Future<void> _register() async {
    ...
    final nacimiento = convertirFecha(nacimientoController.text.trim());
    ...
    if (Localizations.localeOf(context).languageCode == "es") {
      ...
      try {
        ...
        bool exists = await api.search(email);
        ...
        if (exists == true) {
          ...
          await api.register(email, name, nacimiento, pais, telefono, password);
      ...
      if (telefono.length != phoneMaxLength) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              "The phone must have $phoneMaxLength digits for $pais",
            ),
          ),
        );
        return;
      ...
      try {
        ...
        if (exists) {
          ...
          await api.register(email, password, name, nacimiento, telefono, pais);
          ...
          body: SingleChildScrollView(
            ...
            child: Column(
              children: [
                ...
                const SizedBox(height: 16),
                // Nombre
                CustomInput(hint: "Full Name", controller: nameController),
                const SizedBox(height: 16),
